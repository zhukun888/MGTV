<template>
	<view class="home">
		<view class="top">
			{{childlist2.department}}
		</view>
		<view class="bottom">
			<view class="bottom_item" v-for="(item,index) in childlist2.person" :key="index">
				<view class="left">
		...
		<view class="last">
			<view class="last_item" hover-class="change" v-for="(item,id) in childlist2.children" :key="id" @click="navItemClick(item.id)">
				<view class="left">
	...
	export default {
		...
		methods: {
			...
			async getlist() {
				...
				for(var i=0;i<this.childlist1.children.length;i++){
					if(this.id3 == this.childlist1.children[i].id){
						this.childlist2 = this.childlist1.children[i]
					}
				}
			},
		}
